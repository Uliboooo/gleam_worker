...
- **エディタ支援**: Vim キーバインド(設定で切替・遅延ロード)、入力補完(キーワード / スニペット / バッファ内シンボル / stdlib モジュールのメンバ)

> **補完について**: ブラウザ内コンパイラは補完 API を公開していないため、型主導ではない
> ヒューリスティックな補完です(本物の言語サーバーではありません)。stdlib のメンバ補完は、
> コンパイラ用に取得済みの stdlib ソースから `pub fn` シグネチャを解析して提供しています。
